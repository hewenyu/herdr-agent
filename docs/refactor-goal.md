...
2026-09-19 E29 边界回归：任务键按 pi `sessionId` 隔离并兼容同 session 的旧键重试；已解散群的消息和卡片在入口及执行前拒绝进入主 session；旧桥只呈现可接管的 Claude/Codex agent，损坏路由失败关闭。`npm run check` 通过 463/463；这些是本地自动化证据，不关闭真实飞书用户入口和外部资源链路。

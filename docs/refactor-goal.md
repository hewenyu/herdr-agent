...
- **启用 AI 时，普通业务沟通由模型决定。** 程序提供工具及确定性约束，不以关键词分支、固定业务文案或兜底答复代替普通业务决策。用户最新明确例外：主入口 exact `/clear` 是确定性会话命令，不交给模型。飞书审批卡片和无 AI 兼容路径保留确定性操作；Web 只承接本机配置，不承接业务操作。

...
- **主机器人私聊自动压缩长上下文；用户手动 `/clear` 开启新的 pi session。** AI 仍自主选择 session_clear 工具，不增加斜杠关键词执行分支；本轮回复持久后才切换，旧历史/回执保留，已接收排队消息仍属原 session，后续新消息进入新 session。所有群聊不适用；Web 清空暂保留原 session 的 generation 重置语义，不清理任务或 herdr 会话。

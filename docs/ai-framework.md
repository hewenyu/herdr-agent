...
应用私聊处理需求讨论、追问、创建任务及总览，任务群处理绑定任务的反馈、进度和验收。所有自然语言请求
交给模型理解，包括关闭确认；显式斜杠命令保留命令路由。模型选择工具并生成最终答复。
服务完整保存、返回模型的非空成功回复，不通过关键词、正则或
任务快照改写回复，也不在模型失败时生成替代答复。失败只记录安全的错误类别；操作回执继续持久化，防止重放。

启用 AI 后，建群、进展与关闭前的通知也由模型根据结构化生命周期事件决定是否发送及具体内容。
通知模型只有当前任务的查询工具；普通对话中的任务写工具不会授予通知事件。
决策和分片送达分别持久化，模型失败不产生固定替代文案，明确静默与发送失败保持不同状态。

工具用 `execution=not_executed` 区分执行前拒绝，模型可以查询信息并纠正参数；可能已经执行的操作仍受幂等和未知结果保护。
对话答复先保存为待发送，只有全部分片确认后才成为用户已见的历史。模型可用的历史和摘要不会将未确认回复当成已经提出的建议。
模型需要当前进度时调用任务工具；群工具由服务端绑定任务 ID。历史对话原文包含此前可见的操作回复，支持
短回答承接；历史和摘要不构成新的用户授权，也不替代实时状态查询。

`ai.context_tokens` 默认 `50000`，作为包含系统提示、对话、工具定义和本轮工具结果的输入上下文压缩阈值。

...
两份候选均区分了群已创建和初始输入未确认，恢复后的候选还明确“不能声称要求已经转交”。但创建 inbox 最终为 **failed/model_failed**，错误为“pi 调度模型的答复缺少对应工具事实；已登记操作保留，请查询实际状态。”，`outcome=not_executed`。没有对应创建成功回复 outbox，后台建群通知不代替主回合答复。后续按原 checkpoint 重建两次快照，已定位为投递断言误拒：首稿把“已加入”与“初始要求的投递尚未确认”关联；终稿把“群已建立”与“初始要求投递确认前”关联。两者均没有声称已送达。修复按局部分句核验投递断言，真实复验已由 E37 单独记录（见末节），原回合失败不改写。
...
限定结论：用户群内确认之后的 complete→正确阶段答复/两条通知送达→herdr 关闭→群解散子链为 **R-P**。与 E35 的 destroy 通知分别成立；E34 无候选正文的失败历史不被覆盖。E36 创建回合保留原 **R-F**，该缺陷后续修复与独立真实复验见末节，其他入口、身份、审批和未知结果组合继续分验，完整目标 active。


## 后续修复复验（E37，保留本轮失败）

创建答复误拒已由 `772163dc8be0095a4bcacdaa0764731059f5e049` 修复，609 项完整检查、SEA 和三平台 CI 通过。[E37](live-evidence-e37-create-delivery.md) 另一次真实创建回合只有一次 task_create，准确区分群已建/初始投递未确认的答复已送达；随后输出、群确认、两通知和资源清理也有独立证据。E36 的 failed/model_failed 原事实保留；该限定缺陷已修复复验，不能把 E36 原回合改成成功，也不代表其他通知措辞及全部业务组合通过。

...
| B12 完成保留 | `tasks/lifecycle.ts` complete；外部完成确认后保存完成，不关闭 pane/group | `tests/tasks/lifecycle.test.ts` complete 保留与 reopen 后续发送；`tests/tasks/completion-description.test.ts` 未知请求保存不可变描述，只确认 completedAt 不确认描述、不触发重放 | 平台一致性延迟 |
...
| B16 会话记忆 | pi 多 session、代数 clear、可见上下文、原文保留、HTTP 摘要隔离 | `tests/runtime/sessions.test.ts` 重启/clear/延迟确认/操作回执；`tests/runtime/memory.test.ts` 压缩失败/代数竞态；`tests/migration/conversations.test.ts` 旧归档隔离 | 外部 memory 服务的实际 SLA |

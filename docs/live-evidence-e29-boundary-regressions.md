...
- Task IDs and creation locks include the pi `sessionId`, so two sessions can
  reuse a request ID while creating independent projects without serializing
  one session behind the other. A pre-change task key is reused only for a
  retry from its persisted session.
...
and rejects malformed stored routes. `npm run check` passed 464/464 tests,

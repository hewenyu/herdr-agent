...
| 多 pi session | 主机器人私聊长上下文自动压缩；主入口私聊/Web聊天的 exact /clear 不调用模型，事务归档旧 session 并新建选中，成功后仅回复 CLEAR_NEW_SESSION_OK，AI 关闭或模型不可用时也可用；旧历史/回执保留，旧队列不改绑且拒绝执行；Web 显式清空按钮仍为原会话代数重置 | 群聊确定性拒绝；不清任务或原生执行 session；跨入口选择、刷新、重启和迟到回复分别验收，新机械轮转现场证据见矩阵 |

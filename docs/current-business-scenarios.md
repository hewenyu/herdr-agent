...
| 多 pi session | 主机器人私聊长上下文自动压缩；主入口私聊/Web聊天的 /clear 经 pi 工具归档旧 session 并开启新 session，旧历史和回执保留，旧排队消息不改绑且拒绝执行；Web 显式清空按钮仍为原会话代数重置；任务有绑定会话 | 群聊不支持 /clear；不重建或清空原生执行 session；跨入口选择、刷新、重启和迟到回复分别验收 |
...
| /clear | AI 开启时由 pi 调用 session_clear；主入口私聊/Web聊天在当轮回复持久后归档旧 session 并启用新 session | 群聊拒绝；旧历史/回执保留，尚未执行的已接收消息仍绑定旧 session 并因归档拒绝。Web 显式清空按钮仍重置 generation；AI 关闭的兼容分支拒绝执行 |

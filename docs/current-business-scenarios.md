...
| 多 pi session | 创建、命名、切换、清空调度上下文、归档、恢复；任务有绑定会话 | 不重建或清空原生执行 session；需验收跨入口选择、刷新、重启和迟到回复 |
...
| 完成、关闭、销毁和恢复 | 提供独立生命周期动作，群默认保留，可按任务设置清理 | 只清理自己持有的资源；代码/worktree 保留；飞书完成同步与资源清理是异步步骤 |
...
| /clear 的确定性分支 | 代码存在，但 AI 开启时请求先交模型；AI 关闭时该分支拒绝执行 | 需收口帮助与语义：当前可通过 pi 工具或 Web 清空，不应承诺固定 slash 分支直接生效 |

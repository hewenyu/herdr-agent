...
2026-09-19 E38：正式 `v0.3.13` 经真实 Chrome 保存双目录项目、默认 Claude 与默认项目，随后编辑附加目录/默认 Codex，旧任务目录和参与者快照保持；真实飞书创建两个 pi 会话，A1 blocked 时 S2 的新项目 B1 仍建立飞书任务、群与独立 Codex。发现三项 R-F：非法目录错误被配置弹窗遮挡；pi 将 A1 的主目录/双文件输出要求改写，随后更正因阻塞未投递；Claude 目录别名核验失败、Codex 窄屏路径被模型误判，均未自动完成信任。A1 已通过真实外部删群触发 herdr 清理，未代替验收；B1 执行和修复后的真实 UI/需求传递尚待复验。详见 [E38](live-evidence-e38-multi-project.md)，不以建群或离线修复关闭完整矩阵。
...
| WR08 | 页面可用性与分发 | acb/a7 独立二进制加载只读页面，a7 桌面中文长记录截图无明显问题，轮询菜单失焦已实测修复。后续0ccfff2已部署，390×728中文长记录与刷新后Console限定检查通过；全部空态/加载失败/断线界面仍 U；E38 配置弹窗遮挡错误为 R-F，修复尚未部署/真实浏览器复验，随后 cgWindowNotFound 与浏览器连接认证错误阻塞 UI，不能用离线 DOM 回归替代 | O-P / R-部分 |
...
| WR10 | 本机配置错误在当前弹窗内可见，失败保留输入，重试/重开清除旧错误 | E38 v0.3.13 非法附加目录保存被拒，但错误在弹窗后方且不在其无障碍树内（R-F）。源码修复覆盖添加/编辑、删除/Bypass 确认、旧请求晚返回及页面模型错误；客户端事件与服务器测试 10/10，使用轻量 DOM port，不是 Chrome 渲染验收。完整检查 `.cache/e38-check-r2.log` 为 639/639，首轮 638/639 的失败已修，不能覆盖旧部署的现场失败。尚未部署；真实 Chrome 输入保留/错误可见/修改重试/成功关闭/重开无旧错误均待复验 | O-P / R-F（修复后真实复验 U） |

...
2026-09-19 E42 收尾：733 项完整检查、独立 SEA 烟测通过，功能提交 `1d0d7ec` 已构建并重启（PID75496，herdr39037保持）。真实恢复/切换/当前归档、双目录项目、A等待时B运行且C可创建及启动、B/C产物均有独立证据；C业务执行未与B的180秒检查重叠。B/C经真实飞书任务完成API触发生产收尾，A外部删群，三群/执行器均已清理；原12项配置和默认项目恢复。审批卡实际显示正确，但点击及A续行因窗口再次失联仍未验；新补丁仅离线/独立模型复验，PR48继续草稿。详见 [E42记录](live-evidence-e42-resumed-ui.md)。

**继续验收与整改；后续真实 UI 步骤等待窗口恢复，整体目标未完成。** 2026-09-19 E42 验收过程：桌面恢复后已补真实主私聊 S2-R 恢复/切换、自然语言轮转、SB 当前会话延迟归档，以及 A blocked、B 原生 working 时第三项目 C 登记/建任务/群的限定证据；A 首次零写模型失败与重试成功分别保留。15:13 UTC 后飞书/Chrome 再次 `cgWindowNotFound`，浏览器连接器仍报 apikey 鉴权错误；A 卡片已看见但未点击，后续 UI 步骤等待窗口恢复，安全诊断与离线检查继续。上述真实交互时运行 `a3978c0` 的 `0.3.14-dev`；之后诊断与“已申请创建”修复完成 733 项全检并已部署 `1d0d7ec`，部署及收尾见本页顶部。PR #48 仍 draft，完整目标未完成。主目标工具仍返回 `blocked` 且无恢复参数，这是目标状态记录限制；用户继续授权有效，不把本轮再次失去窗口等同于整体停止。详见 [E42 记录](live-evidence-e42-resumed-ui.md)。

2026-09-19 14:00 UTC 阻塞复核（历史记录）：E39 收尾、E40 和本轮连续三次目标回合均无法获取飞书/Chrome 窗口（`cgWindowNotFound`），浏览器连接器另报 `unsupported Codex auth method: apikey`。服务仍 ready，当前零活动任务、零排队/处理中 inbox，原12项项目及默认 herdr-agent保持；独立磁盘满等可推进检查已完成。剩余真实审批显示/点击、修复后新创建答复与通知、session restore/当前归档、多项目及多人业务组合、浏览器可见性不能用API/fixture替代。目标未完成，当前等待恢复可用桌面及真实用户入口；已有授权和全部验收范围保持，PR #48继续草稿，不合并发布。

2026-09-19 E40：复制当前独立二进制，在独立 64 MB HFS+ 卷制造真实 ENOSPC；SQLite/配置文件写入均明确失败且原值保留，释放空间后重试成功、重启保留，数据库完整性通过。测试卷已卸载清理、生产配置与进程未变。准备/完整性探针自身失败分别保留；只补本机配置维护磁盘满子项，不替代飞书业务和其他资源故障。[E40 记录](live-evidence-e40-disk-full.md)。

2026-09-19 E39 进行中：真实 Chrome 已独立复验弹窗错误可见、失败保留输入、成功清错及主目录 Git 初始化；Claude A1 与编辑项目配置后的 Codex A2 均收到准确用户原文并完成限定产物检查，A1 普通审批阻塞未阻止 A2 执行。真实私聊已完成 pi 会话创建/改名/切换及非当前归档；恢复和当前归档仍待验。新增 R-F 为 A1 漏参只读查询恢复后主答复仍失败、A2 提前声称约束已转交、审批卡混入历史编号及开发通知误导；离线修复与原失败分列。`a3978c0` 完整检查 702/702、SEA 和三平台 CI 通过并已部署；后台审批刷新限定通过，API 无法验按钮且桌面再次中断，真实点击仍待验。两个测试群已外部删除并触发 herdr 清理，原默认项目及 12 项 catalog 恢复；清理不冒充用户完成，整体目标 active。详见 [E39](live-evidence-e39-source-approval.md)。

2026-09-19 E38：正式 `v0.3.13` 经真实 Chrome 保存双目录项目、默认 Claude 与默认项目，随后编辑附加目录/默认 Codex，旧任务目录和参与者快照保持；真实飞书创建两个 pi 会话，A1 blocked 时 S2 的新项目 B1 仍建立飞书任务、群与独立 Codex。发现三项 R-F：非法目录错误被配置弹窗遮挡；pi 将 A1 的主目录/双文件输出要求改写，随后更正因阻塞未投递；Claude 目录别名核验失败、Codex 窄屏路径被模型误判，均未自动完成信任。A1 已通过真实外部删群触发 herdr 清理，未代替验收；当时 B1 尚未执行；后续恢复见下一段，真实 UI/新任务需求传递仍待复验。详见 [E38](live-evidence-e38-multi-project.md)，不以建群或离线修复关闭完整矩阵。

2026-09-19 E38 修复后追加：`261771d` 的 `0.3.14-dev` 完整检查 639/639、独立 SEA 烟测通过，已部署。B1 同一旧 Codex 上 pi 自动确认目录、初始 verified、HTML 产物及原生/独立 Python 检查通过，群输出/通知真实 REST 回读，停 review；A1/B1 最终外部删群后均由 herdr 清理，默认项目恢复、原目录保留。仅关闭该旧任务恢复子项，Web 弹窗、新任务原文传递与会话后续操作因桌面不可用继续待验。

更新时间：2026-09-19。状态：**本轮目标未完成，E42 恢复后已补限定证据，后续 UI 步骤再次等待窗口；E21-R2 事件订阅、E22 当前二进制讨论链路、E30 长消息分片与默认收尾、E31 主入口 `/clear` 机械轮转已有限定真实证据，E32 双参与者手动调度及默认清理、E34 创建答复事实恢复已有指定真实证据；E35 已补 destroy 两条通知与清理的限定通过；E32/E34 原通知失败保留，E36 已补 complete 通知限定通过，其创建回合原 R-F 保留，E37 已独立修复复验，其他组合仍分验，其余未覆盖场景未关闭。** 范围继承 [重构目标](refactor-goal.md)、[B/N 需求盘点](node-pi-refactor-requirements.md) 和 [设计](node-pi-design.md)。本文件是新的逐项追踪表；[此前验收](acceptance.md) 保留上一阶段离线证据，不代表本轮真实链路已通过。
...
2026-09-19 E32：真实双参与者 manual 讨论经过群内明确安排 Codex，task_get→participant_send 的 verified 投递、两个 marker、群内用户验收及默认 herdr 关闭两执行器/删除群均有证据。任务 destroyed、两参与者 gone、groupDeleted=true，E32 测试资源已清理。旧“无需操作”通知 R-F 保留；`2900d8b` 本轮后台通知又在 close/delete-group 前声称“已完成收尾”，新增 R-F 不被最终清理覆盖。详见 [E32 记录](live-evidence-e32-manual-discussion.md)。生产身份和 CLI 回放边界见 [E28 身份审计](live-evidence-e28-identity-audit.md)。

2026-09-19 E33：真实飞书主私聊创建无项目、单 Codex 受控讨论，初始 receipt verified、目录信任一次陈旧现场拒绝后自动确认、原生 transcript 零工具调用和指定 marker、署名群输出，以及用户确认后的 herdr close/群删除均有指定任务证据。但完成回合先错用 task ID、后成功查询并完成清理，最终 inbox/turn receipt 却被判为 failed，并称“本轮业务未执行”；该矛盾记为 R-F，不能以清理完成覆盖。创建时“已完整转交”、删群前“已完成收尾”仍需事实修正。详见 [E33 记录](live-evidence-e33-n02-codex.md)。本条旧部署的 PID 54747 曾已退出；后续 E32/E34 运行基线见各自证据，不能混用旧 ready 记录。

2026-09-19 E34：`2900d8b` 的 `0.3.13-dev` 经真实主私聊创建单 Codex 任务后，运行时拦截初始投递未确认却声称“已转交”的草稿，模型再查询并发送正确答复；仅一次 task_create，没有重复创建。用户随后在群内处理普通菜单，目录信任/初始投递、marker、用户验收与资源清理均已完成；受控错误编号明确失败后查询并正确完成真实任务，回合 done/finished，真实 REST 确认远端完成和群 dissolved。详见 [E34 记录](live-evidence-e34-runtime-recovery.md)。后续通知事实护栏及标题边界修正 `0b3c457` 的 557 项自动化检查、SEA烟测通过，10:58 UTC部署为PID98996；真实模型加合成快照探针通过；生产两条收尾通知候选未通过校验并记 unavailable，因被拒正文未持久，不能排除误判，正常通知生成/送达仍未通过，精确 stamp 见 E34。
...
2026-09-19 E36：`2e621e1` 的 `0.3.13-dev`（602 项完整检查、SEA 和三平台 CI 通过）经真实入口创建带引号 Codex 的单参与者受控讨论；旧目录信任现场拒绝后自动确认新现场、initial verified、原生零工具 marker 和群可见输出有证据。群内用户确认后 complete→task_get、两条正确阶段通知 processed/delivered、herdr close、群删除及真实 REST completedAt/group dissolved 的限定完成通知子链 R-P。但主私聊创建回合 inbox failed/model_failed，事实护栏拒绝了两份候选答复，当时疑似误拒，创建阶段保留 R-F；后续修复与独立复验见 E37，不用最终清理改写旧失败。详见 [E36 记录](live-evidence-e36-completion-notices.md)。完整目标 active，未声称全矩阵通过。
...
| B04 | 登记已有项目，验证全部有序目录与默认 agent/default project；任务冻结目录快照 | F1/F2；第二目录无效不先改首目录；A1/A2；bypass仅影响新执行器 | I；O-H projects/config | R-部分：E13非法第二目录拒绝且首目录未初始化Git；E14真实worktree/额外目录读取、原项目干净与额外目录不变通过；完整多目录/配置组合未全验；E38 正式 v0.3.13 真实 Chrome 保存有序双目录/默认 Claude/默认项目，编辑为新附加目录与默认 Codex 后旧 A1 仍保留原目录和 Claude 快照；非法附加目录被拒但错误被弹窗遮挡的历史 R-F 保留；E38 收尾通过配置 API 恢复默认 herdr-agent、只移除两测试登记，原12项/Bypass/目录保留；非浏览器视觉复验；E39 真实 Chrome 添加/编辑非法目录的弹窗错误与输入保留、修正保存、仅主目录 Git 初始化均复验通过；旧 A1 维持 main+extra/Claude，新 A2 实用 main+extra-new/Codex 并完成字节产物核验。全配置组合及浏览器删除登记仍未全验；E42 真实 Chrome 再保存 A 有序 main+extra/默认 Claude，保存后主目录有 .git、附加目录无 .git，原生执行 brief.md 的35字节按序拼接通过；仅指定组合，其他配置及删除仍分验（[E42](live-evidence-e42-resumed-ui.md)） |
| B05 | 明确新项目指令→创建目录/Git/登记；普通新任务只复用项目；连续不同项目独立创建 | 名称穿越/占用拒绝；F3/F4；删除登记不删代码 | I；O-H projects；LIVE-001 | R-部分：E05/E16证据保留。E17 a6018ec B/C各自新项目、真实Codex产物、署名群结果及远端描述通过；原核心输入+新scope限定链路R-P，不覆盖A名称拒绝和历史无工具R-F；E38 A1 blocked 时，S2 的真实私聊 project_create→task_create→task_get 新建 myrix-e38-project-b、远端任务和群，后续独立 Codex 启动；该创建阶段 B1 尚 blocked，无 HTML；后续修复版恢复执行，真实 HTML/Python 检查及飞书群结果回读通过，但不关闭三个项目及全部连续执行组合 |
| B06 | 讨论/开发/评审/测试四类任务各创建；关联要求、参与者、可选群/远端任务；三项目中一项运行/一项blocked不阻塞后续登记 | F1/F3/F4、A1/A2、D1；无平台时保留用户建群意图；显式本地任务能运行 | I；O-H app/tasks/feishu/cli integration | R-部分：E17 C入站/创建时A attention、B原生回合未结束，C独立创建并完成产物/群结果；B/C等待验收后再授权收尾。其他任务类型、所有blocked原因/权限/恢复组合仍分验；E33 补无项目单 Codex 的真实私聊创建/群与远端任务/受控输出，完成回合错误回执 R-F 单列；E36 带引号 Codex 请求实际创建资源，但主私聊创建答复被事实护栏拒，原 R-F 保留，E37 已限定修复复验；E37 同类真实创建答复正确送达且仅一次create，补修复后限定R-P；E38 S1/A1 与 S2/B1 两个开发任务均有独立归属、远端任务/群与执行器；A1 blocked 未阻止 B1 登记及启动。但 A1 原文“两条输入按序写主目录”被 pi 改成“单条输入写附加目录”（R-F），更正未送达、创建回合 uncertain；四类任务、第三项目及修复后的真实飞书全链路仍分验；E38 修复版 B1 随后恢复真实执行并产出HTML，停review；E39 S1/A1 Claude 与 S2/A2 Codex 均由真实私聊创建并收到完整原文、产出指定主目录文件；A1 等待普通审批未阻止 A2 创建执行。A1 主答复漏参恢复误拒、A2 提前称约束已转交另记 R-F；修复后新创建答复、四任务类型及三不同项目组合仍待验；E42 A 首次 projects_list 后模型失败、零写/零任务/无主回复保留 R-F；用户重试只建一次，投递事实候选经只读恢复后正确答复 delivered。15:11:24 A普通审批blocked、B原生working且RUNNING文件时，第三项目C已登记并建远端任务/群；仅并行创建子项 R-部分，C当时starting、创建答复仍processing，后续执行/收尾另验（[E42](live-evidence-e42-resumed-ui.md)）；E42后续原生审计确认B/C均完成指定产物并停review，A仍blocked；C原生启动处于B检查窗口，但C业务working首次观测晚于B已DONE，不声称业务执行重叠；人工验收/清理仍待验 |
| B07 | Claude/Codex 各启动，核对实际 argv、多个目录、bypass和唯一初始 receipt；前任务blocked不阻塞其它执行器启动 | 保留 Bypass；仅当前任务绑定启动目录信任可由专用pi确认，其余群用户选；F3/F5/A3/D3 | I；O-H herdr/lifecycle、tasks | R-部分：E09/E13/E14证据保留。E17 B/C同中文显示名映射不同合法native名称，自动目录信任、唯一初始输入/一次verified投递及真实产物通过；A名称拒绝/误unknown原R-F保留；E33 单 Codex 目录信任先 stale_guard 拒绝再 confirmed、唯一初始 receipt verified 及原生零工具调用已核实；E38 A1 Claude /var 与 /private/var 身份核验返回 directory_trust_scope，B1 Codex 窄屏目录被 pi 误判而未调用确认，真实自动信任均 R-F，Bypass 未替代该确认；修复有 realpath/原生模板/仅受限工具/跨版本未知操作冻结的离线证据及无效果真实模型决策 probe，修复前旧 v0.3.13 的 B1 当时仍 blocked；E38 修复后 B1 同一w24:p1由pi实际工具自动确认，initial verified、原生Python检查与HTML产物通过；E39 A1 的 /var→/private/var Claude 别名已生产复验：旧现场 stale_guard 拒绝，新现场 pi 自动确认，初始投递 verified，双目录原文与产物核对；新 A2 Codex 使用编辑后的目录独立执行。其他 argv/权限/未知写入组合仍分验；E42 A/B/C原文均从真实入站精确保留到原生transcript；A普通审批blocked时B工作，C独立Codex启动，B/C指定原生产物通过并停review。普通审批点击与最终收尾仍待验（[E42](live-evidence-e42-resumed-ui.md)） |
| B08 | 在绑定群续聊/引用、非任务群 @机器人、修订/否定/含糊输入各一例 | A1/A2/A3，F6；完整约束转交、unsupported资源不虚构；D1/D3 | I；O-H app/conversations、runtime | R-部分：E17绑定任务群owner无@ exact /clear拒绝，正文远端/CUA可见，session/selection/status不变且无checkpoint；普通续聊/引用及其他身份组合仍分验；E32 补真实绑定群无@明确安排 Codex，工具只投递给该参与者并送达署名输出；E39 A1 群内续聊要求原生 AskUserQuestion 红/蓝菜单，原文送达同一 Claude 且未自动作答；普通审批解析发生 R-F（见 B11），正确按钮点击后的续聊、引用/修订/否定/含糊及其他身份组合仍待验 |
...
| B10 | Claude/Codex 输出观察、署名与群消息/远端描述/Web历史逐一对应 | F3/F4/F5，D1/D2/D4；控制/通知简洁说用户可见结果，默认无scheduled/持久化/排队回执术语，追问再给诊断；cooldown只合并进度；迟到结果不驱动新任务 | I；O-H transcripts/tasks/app presentation | R-部分：E17 B/C native final、参与者输出、署名群结果及远端描述匹配；E21-R2 的最终描述、结果、收尾通知和飞书已完成列表逐一匹配。B完成回复首句提前称“已完成收尾”为历史R-F；E24 隔离真实组件通知恢复正常、第二样本群内 GET 读到署名与结果，但首样本仍显示 review 字段，简洁措辞与其他异常投递仍分验；E33 marker 署名群 outbox delivered，创建时“已完整转交”和删群前“已完成收尾”仍为事实措辞问题，完成主私聊回合 failed 不算成功答复；E32 双 marker 群输出已核实但后台过早收尾通知仍 R-F；E34 初始投递夸大草稿被拦截、最终纠正答复 delivered；E35 destroy 两条阶段通知生成/送达且 CUA 可见通过，不扩大为 complete 通知通过；E36 补 complete 两条正确阶段通知 delivered/CUA 可见；创建回合错误另记 R-F；E37 创建/完成回复及两收尾通知送达通过，单参与者welcome仍称轮到他人，其他措辞边界继续保留；E38 A1 错误需求同步到远端描述且无参与者产物，原 R-F 保留；修复后隔离真实 Codex 组件收到原文与冲突摘要，精确生成主目录 brief.md 并等待验收，专用 pane 已清理，但合成 inbox/无飞书不能替代群输出验收。E38 阶段长描述截断与单参与者措辞仅有限定离线证据；E38 修复版B1署名HTML结果/待反馈通知已独立REST回读；E39 A1 原文/产物/署名群 UI 与 A2 原文/字节产物已有新任务证据；创建主答复误拒、提前称约束已转交，以及 welcome/paused/链接措辞新增 R-F。修复已部署，保存候选回放与隔离模型探针不代替新创建答复/生产通知复验；长描述及全部输出呈现组合仍未全验 |
| B11 | 目录信任以外的实际 blocked→任务群用户选项→Guard验证→执行；单人/all中断 | A1/A3；回调竞态/双击/过期/换现场；显示裁剪不改完整Guard；D1/D3 | I；O-H approvals/herdr/legacy fixtures | R-部分：E13 普通标记A子项 R-P；E13真实owner/group/card/nonce/key=1 callback done→consumed→native AskUserQuestion答案→群GET输出，8项通过；由用户本人选择，期间directory-trust操作0；其他权限/过期/替换/重复组合未全验；E16补记9a86e40真实pause与native interrupted、重复请求不重复stop；非全部子进程退出证明；E39 A1 AskUserQuestion 误读历史选项为 R-F；修复部署后同 stateSeq 自动停用旧卡并发布新卡，keys/顺序及原生未按键已回读，真实 Feishu GET 只证明卡存在。客户端正确按钮显示/点击和后续执行仍待验，不以后台刷新替代；E42 A 原生 AskUserQuestion 红/蓝与飞书真实卡片红色/蓝色/Type something/Chat about this/Esc呈现已核对；未点击。15:13后窗口再次不可用，不能把可见按钮当作批准/续行通过（[E42](live-evidence-e42-resumed-ui.md)） |
| B12 | `complete`/飞书手动完成确认后默认herdr关闭执行器并解散群；明确keepExecution例外须同时keepGroup:true（无群任务除外），review不收尾；旧默认/未知来源策略在收尾时兼容修正，显式保留证据继续有效 | F3：只回读确认，不重复PATCH；D4；解散前通知/未知删除不重放；群任何原因解散均清对应执行器；明确保留例外单验 | I；O-H tasks lifecycle/completion-description；O-P E15 cleanup-notification：生成不可用审计、未知发送与最后结果屏障 | R-部分：E21-R2 原生完成→事件→herdr close→群 dissolved、全部 outbox delivered 的子场景为 R-P；E15/E16/E17 既有默认/保留/unknown 恢复证据继续有效，E24 真实组件 complete+keepGroup:true 已关闭 pane 并保留群、完成时间独立回读；keepExecution、权限、异常组合及本次飞书用户入口仍未全验；E33 补真实私聊确认→complete done→herdr close done/gone→delete-group done/destroyed；完成回合最终 failed 的矛盾不被清理成功覆盖；E32 真实群验收后两 herdr close 和删群 done、两 gone/task destroyed 已核实，通知过早 R-F 保留；E34 普通审批后真实 marker/用户确认/关闭删除已完成，两个收尾通知候选校验未通过而记 unavailable，不算已送达；E36 群确认后 complete、两通知、herdr close、删群及远端完成子链 R-P，不覆盖创建失败；E42 B/C经真实Feishu任务完成API触发生产事件和默认收尾，close/del-group done、两群dissolved、panes缺失、四通知delivered且时态正确；仅远端API事件链路，非人工UI完成 |
| B13 | 明确 close/外部勾完成→采最后结果→完成同步→关闭通知→资源清理 | F2/F3/F4，A3，D4；远端未确认不清理；模型通知失败不虚报完成 | I；O-H tasks/app | R-部分：E21-R2 已核对事件驱动强制刷新、最终 description sync、远端完成、本地 destroyed、pane gone、群 dissolved 和消息先于删群；E17三任务旧收尾事实保留，B提前声称收尾的历史R-F及其他未知/失败恢复仍需单验；E33 同轮错误 ID 后查找正确任务并完成清理，但 inbox/turn receipt 最终 failed/称未执行，保留新增 R-F；E34 受控错误编号先明确失败、随后正确完成的真实回合 done/finished，补该恢复子项通过，旧 E33 R-F 仍保留 |
| B14 | destroy不代验收，关闭受管资源但保留代码/历史；已销毁拒绝reopen | 部分清理失败恢复；不关闭别的pane/group；F3/F4/A1/A2 | I；O-H tasks/projects | R-部分：E09 外部群解散后只清对应pane、本地destroyed且远端未冒充验收通过；E24 真实组件已验证 destroyed 后显式 destroy+keepGroup:false 及外部解散后 tick 更新，未重启/重关执行器、未改原完成时间且移除失效群链接；显式 destroy 的真实飞书用户入口及其他失败恢复仍未全验；E35 真实 destroy 后 herdr gone、群 dissolved、远端 completedAt=0，无 completion 操作，四条旧历史及空目录保留；无审批记录不算失效验证；E38 为阻止 A1 执行错误要求，经生产应用 REST 外部删群并独立回读 dissolved、completedAt=0，随后本地 destroyed/Claude gone、herdr close done，B1 pane 保留，输入文件未变且无 brief.md；这是外部解散清理而非用户验收或飞书取消指令。审批已在删群前过期且 consumed=false，未验证旧卡片回调；当时 B1 尚未清理；E38 B1产物后外部删群，herdr自动关闭；A1/B1均destroyed/gone，远端仍未完成，非用户群内验收；E39 A1/A2 再经外部删群：两群 dissolved、远端 completedAt=0，生产调度器通过 herdr 关闭两执行器，任务 destroyed/参与者 gone；恢复默认 herdr-agent、原12项/Bypass，输入/产物/历史保留。仅外部清理子项通过，不作用户验收、取消指令或旧卡点击证据；E42 A普通颜色仍未选择时外部删群，远端未完成、herdr关闭、群dissolved；三测试资源最终清理，文件/历史保留，原12项配置恢复；旧卡未真实回放 |
...
| B18 | 单二进制安装、版本核对、服务管理器重启、迁移/回退、长期保留 | 仅桥停止不杀herdr；锁清理、磁盘异常、日志/DB增长、三平台；F4/F7 | I；O-H build/cli/storage，BUILD-001 | R-部分：E17旧基线0b6966f/PID38264/SHAfeb1…0069，345tests、SEA和三平台CI35304636486通过；a6018ec历史证据保留。E17 5d1e96f旧状态副本迁移4项R-local-copy/4项O；完整生产回退、Linux宿主及长期故障未验，旧版本证据保留；E38 261771d的0.3.14-dev通过639项检查与独立macOS SEA烟测并重启，herdr39037保留，版本/SHA见E38；E39 a3978c0 完整702项、独立macOS SEA烟测及CI35446043195三平台通过；build/e39 的0.3.14-dev已重启仅bridge，herdr及两执行器原现场保留，stamp见E39。正式npm仍0.3.13；E40 复制同一SEA在独立HFS+卷完成配置写ENOSPC→原值保持→释放空间重试→重启保留及完整性检查，测试卷清理；服务管理器/回退/Linux宿主及其他长期故障仍未全验；E42 1d0d7ec完整733项、独立SEA烟测通过并重启PID75496，herdr39037保持，SHA/stamp见E42；当前配置模型隔离canary HTTP200/零工具，非生产入站复验 |
| N01 | “当前有哪些任务/谁在等待/我该做什么”真实模型查询工具后答复 | F6，A1/A2；摘要和参与者自述不可替代当前事实 | I；O-H engine/app | R-部分：E01 查询自主调用 tasks_list；E02 多轮查询事实措辞通过（合成回执）；LIVE-001 历史 R-F 保留；E38 e38-before-a-cancel.json 的 ca933e39… checkpoint 实际 tasks_list {} 回读两项 blocked，12:26:03Z 回复及对应 outbox delivered，列出 A1/B1 与各自任务链接；限定默认未结束列表有工具/送达证据，回复仍暴露 blocked 字段，群内范围、all=true、等待原因与行动建议组合未全验；E38 查询答复已用生产应用独立REST读回正文/链接，仍无本轮CUA最终可见性证明 |
| N02 | 明确仅讨论→无项目或已有项目→单Claude/Codex参与需求讨论 | pi不能自行替项目写方案；禁止编码的要求完整保留；D3/D4；F6 | I；O-H tasks/prompts | R-部分：E33 真实私聊无项目单 Codex 受控讨论、禁止开发/原生零工具、初始投递、marker 群输出和用户确认后的资源清理已核实；E30 单 Codex 长输出独立证据保留。E33 完成回合错误判定 R-F；已有项目、单 Claude、实际需求分析及其他组合仍 U；E34 补普通审批后受控输出与完成清理，仍不代表实际需求分析质量通过；E37 无项目单Codex受控marker创建到清理限定通过，不扩大实际讨论质量验收 |
| N03 | Claude+Codex同群轮流讨论，署名可辨、ID可定位；同种多实例另测 | 手动/轮询、轮数/时限、迟到输出、同名拒歧义、未知relay暂停；F3/A2/D4 | I；O-H discussion/app/cli integration | R-部分：E12 R2双方唯一输入零toolcall各一次群输出、1轮暂停通过但首发曾unconfirmed；E13新增首发及移除后resume即时verified通过，旧R-F保留，完整多实例/轮次组合未全验；E32 补真实群内 manual 安排下一位、双 marker 和用户确认后双执行器/群清理，不扩大为自动轮转或同种多实例通过 |
...
| N05 | 主入口切换pi session后继续；多个pi session并行创建不同项目，运行/blocked互不拖住；herdr原生session保持托管 | 已接收排队消息不改道；任务群不可切换；A1/A2，归档与clear分别验证 | I；O-H runtime/app | R-部分：E11 真私聊clear切换后继续成功；两个真实pi session起始相差3ms且各projects_list，处理区间重叠。E11仅证明只读并发；E38 S1/A1 blocked 时 S2 新项目 B1 创建并经修复执行，已有双项目证据。E39 S1/A1 与 S2/A2 分别绑定入站，A1审批等待不阻塞A2；这是同项目编辑前后双任务，改名/切回/非当前归档保持原生绑定。三不同项目、排队改道与恢复完整组合仍待验；E42 A普通审批blocked/B原生working且RUNNING时C登记、建群并启动原生Codex，B/C各自完成产物；SB当前归档后C进新session、B继续工作。C业务执行首次晚于B180秒检查结束，不计执行重叠；排队改道等异常仍待验 |
...
| WR05 | 无 Web 业务操作控件（本机配置除外） | 旧部署页面没有聊天/命令、任务/参与者、审批、清理或 pi session 控件；当前项目/默认/Bypass/模型/身份配置控件属于允许范围，只能调用 allowlist 配置 action，不发送业务消息或 ACK。E38 v0.3.13 真实 Chrome 项目添加/默认设置/编辑已有现场证据；这不覆盖模型连接、Bypass 切换、删除登记和全部配置控件 | O-P / R-部分（仅项目配置指定操作） |
| WR06 | 服务端拒绝业务写入口、保护配置写入口 | `chat.send`、`task.*`、`participant.*`、`approval.*` 和 pi session 等业务 action 必须拒绝；allowlist 配置 action 需合法 Host/Origin/CSRF，未知 action 和坏输入拒绝。当前测试覆盖 `project.save`/`config.ai` 合法 200 及业务 action 403；旧 acb/a7 的11类405结果仅为历史只读部署证据；E38 真实 Chrome 的不存在附加目录被服务端拒绝，合法双目录保存及编辑有 SQLite 回读，但实际错误呈现失败另记 WR10，不视作全部保护分支已现场验收 | O-P / R-部分 |
...
| WR08 | 页面可用性与分发 | acb/a7 独立二进制加载只读页面，a7 桌面中文长记录截图无明显问题，轮询菜单失焦已实测修复。后续0ccfff2已部署，390×728中文长记录与刷新后Console限定检查通过；全部空态/加载失败/断线界面仍 U；E38 配置弹窗遮挡错误为 R-F，修复已部署，尚未真实浏览器复验，随后 cgWindowNotFound 与浏览器连接认证错误阻塞 UI，不能用离线 DOM 回归替代 | O-P / R-部分 |
...
| WR10 | 本机配置错误在当前弹窗内可见，失败保留输入，重试/重开清除旧错误 | E38 v0.3.13 非法附加目录保存被拒，但错误在弹窗后方且不在其无障碍树内（R-F）。源码修复覆盖添加/编辑、删除/Bypass 确认、旧请求晚返回及页面模型错误；客户端事件与服务器测试 10/10，使用轻量 DOM port，不是 Chrome 渲染验收。完整检查 `.cache/e38-check-r2.log` 为 639/639，首轮 638/639 的失败已修，不能覆盖旧部署的现场失败。修复版已部署；真实 Chrome 输入保留/错误可见/修改重试/成功关闭/重开无旧错误均待复验 | O-P / R-F（修复后真实复验 U） |
...
| T15 `sessions_list` | 当前owner活动/archived=true列表 | 群内无此工具；A1；不混淆pi与Codex/Claude原生session；E39 改名/归档前两次真实 sessions_list({}) 回读当前owner会话并定位 S1/S2；archived=true、群内/异owner边界仍待验；E42恢复前真实 archived:true 返回S2-R旧ID/archived:true；群与跨owner异常未覆盖（[E42](live-evidence-e42-resumed-ui.md)） |
| T16 `session_create` | 名称及select布尔，切换仅后续消息 | 不创建task/agent；A2与排队actor，失败不误切；E39 S1/S2 均真实 session_create(name,select:true)，后续 A1/A2 入站各绑新会话；select:false、失败及已排队消息组合仍待验 |
| T17 `session_select` | 在飞书选存在的独立session；Web仅本地查看历史 | 他人/归档/task绑定拒绝，已接收消息不重路由；E39 真实 session_select(S1) 切回已有会话，主回复客户端可见且任务/herdr绑定不变；异owner/归档/task绑定与在途旧消息仍待验；E42显式选S2-R，当前答复仍绑S1并delivered，下一零工具marker在S2-R；异常/在途队列仍分验（[E42](live-evidence-e42-resumed-ui.md)） |
| T18 `session_rename` | 名称与已有session ID，刷新后准确 | 空/超长/外owner拒绝；不改变任务关系；E39 S2→S2-R 的真实工具/持久名称及客户端答复核对，原task/session ID不变；跨目标失败覆盖仅离线修复回归，空/超长/异owner现场仍待验 |
| T19 `session_archive`（本轮新增） | 归档主入口会话；当前回合答复持久后生效，历史/任务/herdr保留 | 当前回合异常不假报成功；异owner/task绑定拒绝；排队消息与迟到ACK不复活归档；E39 非当前 S2-R archived=true，回复 delivered，S1仍选中、任务/herdr绑定保持（SQLite只读回读，未再次观察客户端归档正文）；E39当时当前会话延迟归档及本行异常待验；E42独立纯当前SB归档实际session_archive，scheduled后已archived:true、原回合done/finished/delivered且客户端正文可见；A/B保留，后续C进入新session；不以此前session_clear混算，异常组合仍待验（[E42](live-evidence-e42-resumed-ui.md)） |
| T20 `session_restore`（本轮新增） | 恢复已有主入口归档，随后显式session_select，保留原历史 | 他人/任务绑定拒绝、不新建；Web历史查看不能改变此业务选择；E01真实模型首步正确；E39 仅完成非当前归档，未执行 restore；E39当时恢复原ID/历史、不自动选中及恢复后显式切换的持久效果与本行异常现场待验；E42真实恢复原S2-R ID且未自动选中，再显式select、下一marker入站绑定通过。首次历史hash基线在恢复后，不能证明恢复前后全量历史字节一致；跨owner/任务边界仍待验（[E42](live-evidence-e42-resumed-ui.md)） |

入口差异：基线缺少 `session_archive/session_restore` 工具，盘点期间工作区已新增T19/T20；必须补真实模型自然语言及旧 `/session` 文本的调用验证，不能仅因工具已加入就关闭缺口。普通 pi 工具没有独立 `project_default/catalog_bypass/config.ai`；`project_save(makeDefault)`可设置默认项目。本机 Web/API 已按用户修正保留项目、默认项目、Bypass 与模型配置写入口，受 loopback/Origin/CSRF 保护；任务、讨论、审批及 pi 会话业务仍从飞书发起。普通会话没有任意shell/编辑/人工审批工具是职责边界。另有专用启动上下文的 directory_trust_confirm，仅确认绑定目录的原生信任菜单；不计入上述20个普通工具，不作为通用审批能力。
...
| 编号 | 检查项 / 判定 | 下一步与关闭证据 |
| --- | --- | --- |
| GAP01 / P0 | 真实模型没用工具却声称执行成功（LIVE-001） | E17原核心输入+新scope的B完整创建/产物/结果可见限定链路通过，C独立请求并行完成；不覆盖原历史上下文和A启动失败。新收尾措辞R-F仍需部署后独立验证，整体目标不关闭；E34 真实任务创建只有一次，投递夸大草稿触发只读恢复后发送正确答复，其他语义和原历史故障仍分验；E36 有真实工具和资源却创建答复 failed，事实护栏误拒已由 E37 限定修复复验，原故障不以最终清理覆盖 |
| GAP02 / P0 | 成功定义过度依赖inbox done/outbox delivered | E17 B/C各阶段及最终清理已分别核验；B首句“已完成收尾”发生在群解散前，新增措辞R-F保留。E18提示修正已部署、真实受控probe单样本通过，尚未生产重验；不以最终成功覆盖先前夸大；E32 再次提前声称收尾的 R-F 与后续 e71deb7 通知护栏离线修正分列；E34 主回合恢复通过不代表后台通知全通过；E34 两条收尾候选校验未通过，正文未持久不能排除误判，正常通知生成/送达仍未通过；E35 已补 destroy 的两条正确阶段通知 delivered，E34 缺正文原失败保留，complete 全路径仍待复验；E36 补 complete 两通知子链 R-P，创建回合新增失败和E34旧正文缺失仍保留 |
...
| GAP17 / P2 | 长期与资源故障 | E11双真实模型只读并发已证；旧CLI已限定验证损坏SQLite拒绝并保留原文件。E40补独立卷真实ENOSPC下SQLite/配置文件写失败不虚报、原值保持、释放空间重试与重启保留、完整性ok；测试脚本失败与清理分列。业务写入中途磁盘满、大任务量、其他DB损坏/日志增长/WS抖动仍未验，不扩大为长期稳定 |
| GAP18 / P1 | 飞书业务与 Web 配置/历史边界 | 旧W23/W28/W29业务表单需求撤销；任务业务缺口在飞书/pi工具中核对，Web/API拒绝业务写入。用户明确保留的本机项目、默认项目、Bypass和模型配置入口继续可写；历史浏览与诊断只读 |

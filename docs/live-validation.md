...
更新时间：2026-09-19。状态：**本轮目标仍进行中；E21-R2 事件订阅、E22 当前二进制讨论链路、E30 长消息分片与默认收尾、E31 主入口 `/clear` 机械轮转已有限定真实证据，E32 双参与者手动调度及默认清理、E34 创建答复事实恢复已有指定真实证据；E32 通知 R-F 与 E34 普通审批/输出/收尾未验仍保留，其余未覆盖场景未关闭。** 范围继承 [重构目标](refactor-goal.md)、[B/N 需求盘点](node-pi-refactor-requirements.md) 和 [设计](node-pi-design.md)。本文件是新的逐项追踪表；[此前验收](acceptance.md) 保留上一阶段离线证据，不代表本轮真实链路已通过。
...
2026-09-19 E34：`2900d8b` 的 `0.3.13-dev` 经真实主私聊创建单 Codex 任务后，运行时拦截初始投递未确认却声称“已转交”的草稿，模型再查询并发送正确答复；仅一次 task_create，没有重复创建。普通 Codex 更新菜单审批已发到任务群，仍待用户处理，参与者 marker、验收与清理未通过。详见 [E34 记录](live-evidence-e34-runtime-recovery.md)。后续通知事实护栏及标题边界修正 `0b3c457` 的 557 项自动化检查、SEA烟测通过，10:58 UTC部署为PID98996；真实模型加合成快照探针通过，生产收尾通知复验仍待用户处理E34普通菜单后进行，精确stamp见E34。
...
| N02 | 明确仅讨论→无项目或已有项目→单Claude/Codex参与需求讨论 | pi不能自行替项目写方案；禁止编码的要求完整保留；D3/D4；F6 | I；O-H tasks/prompts | R-部分：E33 真实私聊无项目单 Codex 受控讨论、禁止开发/原生零工具、初始投递、marker 群输出和用户确认后的资源清理已核实；E30 单 Codex 长输出独立证据保留。E33 完成回合错误判定 R-F；已有项目、单 Claude、实际需求分析及其他组合仍 U |
...
| GAP02 / P0 | 成功定义过度依赖inbox done/outbox delivered | E17 B/C各阶段及最终清理已分别核验；B首句“已完成收尾”发生在群解散前，新增措辞R-F保留。E18提示修正已部署、真实受控probe单样本通过，尚未生产重验；不以最终成功覆盖先前夸大；E32 再次提前声称收尾的 R-F 与后续 e71deb7 通知护栏离线修正分列；E34 主回合恢复通过不代表后台通知全通过 |

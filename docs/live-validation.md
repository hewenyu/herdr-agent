...
**当前阻塞，目标未完成。** 2026-09-19 14:00 UTC 阻塞复核：E39 收尾、E40 和本轮连续三次目标回合均无法获取飞书/Chrome 窗口（`cgWindowNotFound`），浏览器连接器另报 `unsupported Codex auth method: apikey`。服务仍 ready，当前零活动任务、零排队/处理中 inbox，原12项项目及默认 herdr-agent保持；独立磁盘满等可推进检查已完成。剩余真实审批显示/点击、修复后新创建答复与通知、session restore/当前归档、多项目及多人业务组合、浏览器可见性不能用API/fixture替代。目标未完成，当前等待恢复可用桌面及真实用户入口；已有授权和全部验收范围保持，PR #48继续草稿，不合并发布。
...
| B04 | 登记已有项目，验证全部有序目录与默认 agent/default project；任务冻结目录快照 | F1/F2；第二目录无效不先改首目录；A1/A2；bypass仅影响新执行器 | I；O-H projects/config | R-部分：E13非法第二目录拒绝且首目录未初始化Git；E14真实worktree/额外目录读取、原项目干净与额外目录不变通过；完整多目录/配置组合未全验；E38 正式 v0.3.13 真实 Chrome 保存有序双目录/默认 Claude/默认项目，编辑为新附加目录与默认 Codex 后旧 A1 仍保留原目录和 Claude 快照；非法附加目录被拒但错误被弹窗遮挡的历史 R-F 保留；E38 收尾通过配置 API 恢复默认 herdr-agent、只移除两测试登记，原12项/Bypass/目录保留；非浏览器视觉复验；E39 真实 Chrome 添加/编辑非法目录的弹窗错误与输入保留、修正保存、仅主目录 Git 初始化均复验通过；旧 A1 维持 main+extra/Claude，新 A2 实用 main+extra-new/Codex 并完成字节产物核验。全配置组合及浏览器删除登记仍未全验 |
...
| B06 | 讨论/开发/评审/测试四类任务各创建；关联要求、参与者、可选群/远端任务；三项目中一项运行/一项blocked不阻塞后续登记 | F1/F3/F4、A1/A2、D1；无平台时保留用户建群意图；显式本地任务能运行 | I；O-H app/tasks/feishu/cli integration | R-部分：E17 C入站/创建时A attention、B原生回合未结束，C独立创建并完成产物/群结果；B/C等待验收后再授权收尾。其他任务类型、所有blocked原因/权限/恢复组合仍分验；E33 补无项目单 Codex 的真实私聊创建/群与远端任务/受控输出，完成回合错误回执 R-F 单列；E36 带引号 Codex 请求实际创建资源，但主私聊创建答复被事实护栏拒，原 R-F 保留，E37 已限定修复复验；E37 同类真实创建答复正确送达且仅一次create，补修复后限定R-P；E38 S1/A1 与 S2/B1 两个开发任务均有独立归属、远端任务/群与执行器；A1 blocked 未阻止 B1 登记及启动。但 A1 原文“两条输入按序写主目录”被 pi 改成“单条输入写附加目录”（R-F），更正未送达、创建回合 uncertain；四类任务、第三项目及修复后的真实飞书全链路仍分验；E38 修复版 B1 随后恢复真实执行并产出HTML，停review；E39 S1/A1 Claude 与 S2/A2 Codex 均由真实私聊创建并收到完整原文、产出指定主目录文件；A1 等待普通审批未阻止 A2 创建执行。A1 主答复漏参恢复误拒、A2 提前称约束已转交另记 R-F；修复后新创建答复、四任务类型及三不同项目组合仍待验 |
...
| T15 `sessions_list` | 当前owner活动/archived=true列表 | 群内无此工具；A1；不混淆pi与Codex/Claude原生session；E39 改名/归档前两次真实 sessions_list({}) 回读当前owner会话并定位 S1/S2；archived=true、群内/异owner边界仍待验 |
...
| T17 `session_select` | 在飞书选存在的独立session；Web仅本地查看历史 | 他人/归档/task绑定拒绝，已接收消息不重路由；E39 真实 session_select(S1) 切回已有会话，主回复客户端可见且任务/herdr绑定不变；异owner/归档/task绑定与在途旧消息仍待验 |
...
| T19 `session_archive`（本轮新增） | 归档主入口会话；当前回合答复持久后生效，历史/任务/herdr保留 | 当前回合异常不假报成功；异owner/task绑定拒绝；排队消息与迟到ACK不复活归档；E39 非当前 S2-R archived=true，回复 delivered，S1仍选中、任务/herdr绑定保持（SQLite只读回读，未再次观察客户端归档正文）；当前会话延迟归档及本行异常仍待验 |
| T20 `session_restore`（本轮新增） | 恢复已有主入口归档，随后显式session_select，保留原历史 | 他人/任务绑定拒绝、不新建；Web历史查看不能改变此业务选择；E01真实模型首步正确；E39 仅完成非当前归档，未执行 restore；恢复原ID/历史、不自动选中及恢复后显式切换的持久效果与本行异常现场仍待验 |

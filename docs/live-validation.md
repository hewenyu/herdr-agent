...
2026-09-19 E32：真实飞书创建双参与者 `manual` 讨论，Claude/Codex 的新目录信任提示均由 pi 自动确认；Claude 的普通 Bypass 审批卡真实发送到任务群并等待用户。旧二进制在 `initialSent=false` 且任务处于 `blocked` 时仍发出“后续会自动推进、无需你操作”的通知，已判为 R-F；当前分支已修正提示词并通过 12 项定向测试、lint 和 470 项完整检查。详见 [E32 记录](live-evidence-e32-manual-discussion.md)。新二进制重启后仍需完成用户审批、双 marker、任务完成和群/执行器清理复验。生产身份和 CLI 回放边界见 [E28 身份审计](live-evidence-e28-identity-audit.md)。

2026-09-19 E33：真实飞书主私聊创建无项目、单 Codex 受控讨论，初始 receipt verified、目录信任一次陈旧现场拒绝后自动确认、原生 transcript 零工具调用和指定 marker、署名群输出，以及用户确认后的 herdr close/群删除均有指定任务证据。但完成回合先错用 task ID、后成功查询并完成清理，最终 inbox/turn receipt 却被判为 failed，并称“本轮业务未执行”；该矛盾记为 R-F，不能以清理完成覆盖。创建时“已完整转交”、删群前“已完成收尾”仍需事实修正。详见 [E33 记录](live-evidence-e33-n02-codex.md)。E32 最新只读状态为 Claude 已投递并输出、Codex 尚未初始投递、任务 review，双执行器仍保留；旧 PID 54747 已不存在，不能继续声称当前服务 ready。
...
| B10 | Claude/Codex 输出观察、署名与群消息/远端描述/Web历史逐一对应 | F3/F4/F5，D1/D2/D4；控制/通知简洁说用户可见结果，默认无scheduled/持久化/排队回执术语，追问再给诊断；cooldown只合并进度；迟到结果不驱动新任务 | I；O-H transcripts/tasks/app presentation | R-部分：E17 B/C native final、参与者输出、署名群结果及远端描述匹配；E21-R2 的最终描述、结果、收尾通知和飞书已完成列表逐一匹配。B完成回复首句提前称“已完成收尾”为历史R-F；E24 隔离真实组件通知恢复正常、第二样本群内 GET 读到署名与结果，但首样本仍显示 review 字段，简洁措辞与其他异常投递仍分验；E33 marker 署名群 outbox delivered，创建时“已完整转交”和删群前“已完成收尾”仍为事实措辞问题，完成主私聊回合 failed 不算成功答复 |

...
更新时间：2026-09-19。状态：**本轮目标仍进行中；E21-R2 事件订阅、E22 当前二进制讨论链路、E30 长消息分片与默认收尾、E31 主入口 `/clear` 机械轮转已有限定真实证据，E32 双参与者手动调度及默认清理、E34 创建答复事实恢复已有指定真实证据；E35 已补 destroy 两条通知与清理的限定通过；E32/E34 原通知失败保留，E36 已补 complete 通知限定通过，但创建回合新增 R-F，其他组合仍分验，其余未覆盖场景未关闭。** 范围继承 [重构目标](refactor-goal.md)、[B/N 需求盘点](node-pi-refactor-requirements.md) 和 [设计](node-pi-design.md)。本文件是新的逐项追踪表；[此前验收](acceptance.md) 保留上一阶段离线证据，不代表本轮真实链路已通过。
...
| N02 | 明确仅讨论→无项目或已有项目→单Claude/Codex参与需求讨论 | pi不能自行替项目写方案；禁止编码的要求完整保留；D3/D4；F6 | I；O-H tasks/prompts | R-部分：E33 真实私聊无项目单 Codex 受控讨论、禁止开发/原生零工具、初始投递、marker 群输出和用户确认后的资源清理已核实；E30 单 Codex 长输出独立证据保留。E33 完成回合错误判定 R-F；已有项目、单 Claude、实际需求分析及其他组合仍 U；E34 补普通审批后受控输出与完成清理，仍不代表实际需求分析质量通过 |
...
| 编号 | 检查项 / 判定 | 下一步与关闭证据 |
| --- | --- | --- |
| GAP01 / P0 | 真实模型没用工具却声称执行成功（LIVE-001） | E17原核心输入+新scope的B完整创建/产物/结果可见限定链路通过，C独立请求并行完成；不覆盖原历史上下文和A启动失败。新收尾措辞R-F仍需部署后独立验证，整体目标不关闭；E34 真实任务创建只有一次，投递夸大草稿触发只读恢复后发送正确答复，其他语义和原历史故障仍分验；E36 有真实工具和资源却创建答复 failed，疑似事实护栏误拒正在定位，原故障未以最终清理覆盖 |

...
| B16 | 助手会话与记忆 | 私聊和任务群隔离；压缩旧上下文并保留近期对话及归档；私聊 `/clear` 切换助手会话，不清任务、编码 session 或操作回执。任务群不支持 `/clear` |
...
| `/clear` | 仅主应用私聊新开助手上下文 | 映射为 pi session 的明确操作；名称与影响待定，不能清除编码 session 或回执 |

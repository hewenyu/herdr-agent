...
下文 Go 基线的 Web 项目管理、聊天、配置建议及 D08 旧问题只作历史盘点；其 Web 写能力已撤销，不再是待实现需求。当前 B/N 业务能力须改由飞书交互承接。
...
| 本项目的确定性服务 | 身份与绑定校验、工具执行、状态持久化、事件去重、投递回执、恢复、飞书业务接入与 Web 只读历史 | 这些事实和约束由程序维护，不能只依赖 pi 的对话记忆或提示词 |
...
| `/projects` | 查看配置项目 | 当前由飞书 pi 查询承接；旧 Web 项目页建议已撤销，命令是否主推另定 |
...
| D08 | 历史问题：旧 pane 桥/无 AI 模式，以及 Web 承担何种职责 | Web 范围已由2026-09-18用户定案：仅只读会话记录，无配置/聊天/业务管理；其余兼容能力按当前设计与飞书验收明确 |
...
以下历史场景能力仍需验收，但业务入口统一为真实飞书；Web 写流程撤销，仅验只读历史及禁止写入。本段不宣称这些端到端操作已经通过。建议使用现有测试中的故障样例作为迁移依据，而不是只比较函数名称。

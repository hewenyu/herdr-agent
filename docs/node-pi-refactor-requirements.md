...
| B16 | 助手会话与记忆 | 私聊和任务群隔离；自动压缩旧上下文并保留原始历史；主入口飞书私聊/Web聊天的 `/clear` 由 pi 调用工具，在当轮答复持久后归档旧 pi session、创建并选中新 session；旧历史和操作回执保留，旧排队消息不改绑且因归档拒绝执行。不清任务或 herdr 托管的编码 session；群聊不支持 `/clear` |
...
| `/clear` | 主入口飞书私聊/Web聊天归档旧 pi session 并开启新会话 | 由模型选择 `session_clear`；当轮答复持久后归档旧 session、创建并选中新 session，保留旧历史和回执；旧排队消息不改绑且拒绝执行。所有群聊不支持；不清任务或编码 session。Web 显式清空按钮仍独立使用原 session generation 重置 |

...
test("current source wraps direct followup, source mutation conflicts, automatic relay never reuses followup", async () => {
  ...
  try {
    ...
    // The earlier automatic relay is still running on p2. Settle that native
    // turn before testing a new relay; an idle-target safety gate must remain.
    h.herdr.finish("p2", "已完成上一轮讨论");
    await h.service.tick();
...
test("main-private followups and lifecycle instructions retain exact task-scoped user revisions", async () => {
  const h = setup();
  ...
  try {
    ...
    await h.service.tick();
    const sender = { ...feishuActor, messageId: "private-update" };
    ingress(h, sender, "这次只检查，不要修改任何文件");
    await h.service.send(sender, task.id, task.participantIds[0], "检查项目");
    const pauser = { ...feishuActor, messageId: "private-pause" };
    ingress(h, pauser, "暂停这个任务");
    await h.service.action(pauser, task.id, "pause");
    const revisions = h.store.list<{ taskId: string; source: { text: string } }>(
      "task_user_revisions",
    );
    assert.equal(revisions.length, 2);
    assert.ok(revisions.every((entry) => entry.taskId === task.id));
    assert.deepEqual(
      revisions.map((entry) => entry.source.text).sort(),
      ["暂停这个任务", "这次只检查，不要修改任何文件"].sort(),
    );
  } finally {
    h.close();
  }

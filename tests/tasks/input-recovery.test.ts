...
test("uncertain first relay recovers the unique fingerprint, preserves pause and captures reply once", async () => {
  const outputs: string[] = [];
  const f = setup({
    output: async (_task, _participant, entry) => {
      outputs.push(entry.text);
  ...
  try {
    ...
    (f.herdr as HerdrPort).initialInput = async () => f.herdr.sends[1]?.text;
    const restored = new TaskService(f.options);
    await restored.tick();
    assert.equal(f.store.get<OperationReceipt>("operations", key)?.state, "done");

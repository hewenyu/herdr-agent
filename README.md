...
PR #35 and PR #37 add the future business-claim and write-evidence guards with regression coverage; they are included in `v0.3.11` together with the earlier tool-evidence checks. The merged source is `master` at `2a5bbd0`; the published release and npm package use the same commit. The root `package.json` is the private source package and remains at version `0.3.0`; it is not the published npm entry package. The currently running local binary, when present, must be checked with `version --json`; a `dev/unknown` stamp is development evidence and is not a release or commit match.

...
At startup, pi automatically confirms only the native Claude/Codex directory-trust prompt when the directory matches the authorized task project. In `manual` discussions pi only attempts the first participant automatically; later participants wait for the user or scheduler, and a `blocked` participant means the user must handle the approval card in the task group. `round_robin` may advance only after the previous participant has produced a verified output. Other approval prompts remain in the task group for the user to choose explicitly. The project/task Bypass setting is preserved as an explicit option and is never enabled implicitly by this startup handling.

...
The tool-claim recovery changes from PR #28 and the legacy tool-count guard from PR #31 are included in `v0.3.8`. The current source also contains outcome-aware evidence checks described above; publish a new `v0.3.x` tag before treating those additional checks as part of a release. The root `package.json` is the private source package and remains at version `0.3.0`; it is not the published npm entry package. The local SEA evidence below was rebuilt from merge commit `452aae9` and restarted as `v0.3.7` (PID `31577`, SHA256 `f717b402b2cc82f6fc815046d0b20eef4287c751238cb5593a2f0a4a9a412aec`).

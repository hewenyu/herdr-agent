...
The latest published release is [v0.3.9](https://github.com/hewenyu/herdr-agent/releases/tag/v0.3.9). The public entry package and all three native optional packages are published at the same version. Install the entry package with `npm install -g @yuebanlaosiji/myrix@0.3.9`, or use `@latest` for the current stable version. The npm launcher and the release archives expose the same `myrix` command and the `herdr-agent` compatibility alias.

The tool-claim recovery changes from PR #28, the legacy tool-count guard from PR #31, and the outcome-aware evidence checks released in `v0.3.9` are published in that version. This checkout's follow-up branch is at `d6bc5ac`; it also adds regression coverage for first-person promises such as “I will create a project” and “我会创建项目”. That follow-up is pending its own release and must not be inferred from the already published `v0.3.9`. The root `package.json` is the private source package and remains at version `0.3.0`; it is not the published npm entry package. The currently running local binary, when present, must be checked with `version --json`; a `dev/unknown` stamp is development evidence and is not a release or commit match.

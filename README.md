...
Full live acceptance remains in progress. Automated checks and packaging smoke tests do not establish that every real Feishu, model and herdr scenario works. [Live validation](docs/live-validation.md) records passed, partial, untested and failed cases; [E24](docs/live-evidence-e24-retained-group.md) records retained-group cleanup and notification component checks, [E25](docs/live-evidence-e25-transport-legacy.md) records Feishu transport recovery and legacy-route validation, [E26](docs/live-evidence-e26-feishu-rest-lifecycle.md) records a real REST task/group lifecycle with its permission gap, [E27](docs/live-evidence-e27-service-ingress-prompt.md) records the current service bot's ingress prompt, [E28 model probe](docs/live-evidence-e28-real-model-tool-decision.md) records a real model first-tool decision probe, [E28 identity audit](docs/live-evidence-e28-identity-audit.md) records the production app/user identity boundary and read-only scope limits, [E29](docs/live-evidence-e29-boundary-regressions.md) records local session/group boundary regressions, [E30](docs/live-evidence-e30-long-message.md) records a real long-message split and cleanup, [E31](docs/live-evidence-e31-clear-command.md) records a real main-session `/clear` rotation, and [E32](docs/live-evidence-e32-manual-discussion.md) records the manual discussion notification correction. In E32, a real task-group request subsequently delivered the Codex instruction; both markers were observed, the user accepted the task, and both herdr execution targets and the group were closed. A background notice still claimed cleanup was complete too early; that failure remains recorded. Ordinary approvals remain user actions.

[E33](docs/live-evidence-e33-n02-codex.md) records a real Feishu request for a discussion without a project and with one Codex participant: its exact output, zero native tool calls, group delivery and cleanup after user confirmation were verified. However, the completion turn was ultimately marked failed and incorrectly reported that no business action had executed, despite successful cleanup. This remains a failure, not a completed acceptance case.

A locally accepted or queued task does not prove that its remote task, group or initial participant input exists or has been delivered. [E34](docs/live-evidence-e34-runtime-recovery.md) verified that the deployed first revision intercepted an unsupported “requirements forwarded” claim, queried the task again and delivered a corrected reply without creating it twice. E34 remains pending at a normal Codex update-menu approval in its task group; participant output and cleanup are not yet accepted. The lifecycle-notice guard, including task-title handling, passed 557 automated tests and was deployed as `0.3.13-dev` from `0b3c457`; a real-model probe with a synthetic snapshot also passed. Its production cleanup-notice recheck remains pending. E33's completion-error recovery also still needs a dedicated live recheck. These records cover specific scenarios and do not establish full acceptance.
...
Task identity and creation locks are scoped to the selected pi session. Reusing a request or message ID in another session therefore creates an independent task and does not serialize unrelated project creation. A dissolved task group remains available for history, but late messages and card callbacks are rejected at ingress and before inbox execution; they cannot fall back to the main pi session or consume an approval. When Feishu reconnects, the previous task scheduler is stopped before the replacement starts reconciling the same records, so an old connection cannot continue acting on the new connection's work.

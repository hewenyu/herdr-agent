...
需要切换话题时，在主应用入口私聊内单独发送 `/clear`，收到确认后再发送新需求。
任务群不支持 `/clear`；每个任务群始终保留独立会话，通过自动压缩和记忆管理上下文。
入口私聊助手的旧对话和摘要归档到本地，不再进入新会话上下文；任务、任务群、编码 agent 会话和
操作去重记录保留。该命令要求启用 AI，不会清空 Codex/Claude 自身的上下文，也不保证新任务一定创建成功。
`/clear` 不接受参数；不要把新需求与命令放在同一条消息中。
...
| `/clear` | 仅在主应用私聊中刷新助手会话，保留各任务群及编码 agent 会话 |

...
启用[AI 任务入口](#用自然语言操作任务)后，可以直接私聊本项目机器人，用自然语言创建任务、查询进度和补充要求。
...
启用 `[tasks]`、补齐下文权限后，启动 `herdr-agent serve`，即可在本机打开
**http://127.0.0.1:18790/** 配置页面。

页面支持添加、编辑项目、选择默认项目和 agent，以及按顺序配置一个项目的多个文件夹。第一个目录作为
工作目录，其余目录通过 `--add-dir` 传给 Codex 或 Claude；已有目录不要求是 Git 仓库。保存后，新任务
...
`serve --config-listen 127.0.0.1:18791` 可以更换本机监听地址，`serve --no-config-ui` 可以关闭页面。
...
用户各自的目录。新项目是普通目录，不自动初始化 Git 或创建 worktree，也不自动接管已存在的同名目录。
...
已有应用缺少这些权限时，先停止桥，然后运行本项目命令：

```sh
herdr-agent setup --update-permissions
```

它会为当前应用生成并打开新的权限确认 URL；已有凭据不会跳过这次更新。也可以加 `--app cli_…` 指定
应用。页面确认完成后凭据立即保存，再按提示完成消息和按钮验证；应用 ID 不一致时会拒绝覆盖原配置。
更新完成后重新启动 `herdr-agent serve`。也可以在开发者后台按租户审批和发布流程手动开通。
修改 `config.toml` 不会自动授予权限；基础 `setup` 的消息和按钮验证也不代表任务流程已经验证。机器人是它创建的任务群的群主。飞书任务同时把你和应用设为负责人，完成模式
...
3. agent 就绪后发送初始要求。需要处理启动确认或权限请求时，使用已有的屏幕卡片；后续补充要求、回复和
   进展都留在对应任务群。
4. 看完结果并验收后，在飞书任务面板勾选完成，或在任务群执行 `/task complete`。继续追加要求前，在面板
   重新打开任务，或执行 `/task reopen`。
5. 不再需要会话时，执行 `/task destroy`，关闭该任务的执行 pane 并解散任务群。**飞书解散群后不保留群
   聊天记录。** 桥会先把最新任务摘要写回飞书任务；本地代码和飞书任务记录保留。

完成与销毁是两个操作。勾选完成不会停止进程或删群；销毁会话也不会把尚未完成的任务自动勾选完成。已经销毁
...
任务创建后有对应飞书任务和私有任务群。群内仍直接与该任务的 Codex/Claude 对话，启动确认和审批也在
群内处理；入口私聊负责跨任务的自然语言管理。运行状态、最近回复和更新时间写回任务，机器人查询后
可以总结哪些任务在进行、哪些等你处理。agent 一轮结束不代表任务已经验收完成；勾选完成和销毁会话
仍是两个独立操作。
...
卡片回调、镜像。普通 agent 使用配置中的推送目标；托管任务使用通过授权检查后创建并持久绑定的私有任务群。
...
保存工具操作回执，`conversations/` 保存按用户和聊天隔离的对话与消息回执，文件权限为 0600；模型上下文
保留最近 20 轮完整对话，进度查询仍读取实际任务。普通镜像开关仍只活在内存里，重启后回到

...
系统按 UTF-8/JSON 字节和消息、工具结构开销保守估算 tokens，计入系统指令、对话、工具定义及工具结果。
达到阈值时，Eino 自动摘要旧上下文并保留近期问答。模型实际上下文窗口还需留出额外 `4096` tokens
用于输出；修改此配置不会扩大模型本身的窗口。不再按固定 40 条消息静默丢弃历史。
压缩失败时保留已有历史并记录错误类别；模型明确标记为不完整或截断的输出不能作为成功摘要。
...
入口私聊支持需求沟通、澄清、创建任务和任务总览。所有自然语言请求都交给模型理解、选择工具和答复，
成功回复完整保留原文，包括“确认关闭”等自然语言也由模型理解；显式斜杠命令保留原有命令路由。
明确新建项目但未命名时，模型可自行选名执行，也可在必要时追问；
回复“就叫 pelican-bike-svg”能承接前面的创建要求，重启后也能继续已有对话。已有任务不会拦截新的创建请求。
创建任务后，入口接收简短回执和就绪后的任务群入口，不重复接收执行进展、审批卡片或待验收通知。
具体实现反馈、单个任务的进度和验收，在对应任务群直接沟通。主动在入口询问时，仍可查询任务总览。
模型选择查询范围，`herdr_list` 默认只返回未结束任务，`all=true` 包含已完成、已销毁记录。
服务不根据自然语言规则覆盖工具参数或改写模型答复，数据归属仍由工具层校验。
...
对应 Codex/Claude，明确验收结单则完成任务并关闭会话。提示词要求模型区分否定、修改反馈和明确验收。
...
进度由模型调用工具查询并生成答复；提示词要求区分 agent 自述、目录现况和已验证事实，
首次要求未确认送达时应明确说明，agent 一轮结束不等于用户已验收。所有历史可见回复原文保留，不能作为当前执行证据。
模型、记忆或预算失败时只记录安全错误类别，不发送固定兜底或旧任务快照；工具操作回执仍单独持久化，避免重放。

启用 AI 后，任务群欢迎、进展和关闭前通知也由模型决定是否发送及具体内容。代码提供事件、受限查询工具和送达记录。
执行前校验拒绝允许模型纠正参数；未知操作结果继续受幂等保护。未确认完整送达的回复不会成为模型后续承接的建议。
任务最终结果按完整正文发送，成功分片与完整送达回执持久化，重启后跳过已确认内容。
验证命令及真实联调边界见[模型回复与投递验证](docs/model-reply-validation.md)。
运行状态、最近回复和更新时间继续写回飞书任务，供其他有权限的 AI 总结。
...
模型旧上下文达到配置阈值后自动摘要，自然语言进度查询由模型调用工具读取实际任务。普通镜像开关仍只活在内存里，重启后回到

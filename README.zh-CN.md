...
等待授权期间，**http://127.0.0.1:18790/** 仍可访问。页面顶部显示授权状态、缺少的权限、当前登录链接；程序会自动打开本地页面一次，同时在日志中打印授权 URL。使用 `--no-config-ui` 时从日志获取
链接，程序也会尝试直接打开授权页。链接过期后会重新检查权限并自动生成新链接；登录确认后还会再次
...
**http://127.0.0.1:18790/** 在等待授权期间也可使用。

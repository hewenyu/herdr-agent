...
新建 AI 任务默认由 pi 根据真实执行结果持续调度：自主分派、复核、返工并委托参与者汇总交付，不再要求每一步人工接力。历史任务及显式手动/轮转策略保留原行为；交付仍等待用户验收，普通审批仍由用户选择。中断、投递核验、预算和本分支验证边界见[持续调度审计](docs/ai-orchestration-audit-2026-09-25.md)。
...
AI 新建任务默认使用模型调度，每次授权接续默认最多 32 次决策、240 分钟；显式 `round_robin` 讨论仍默认最多 4 轮、30 分钟。可以指定参与者或暂停。多个同种模型实例有不同 participant ID。普通文本不是权限菜单批准，审批使用实际卡片/屏幕选项；结果未知时不能自动重发。
